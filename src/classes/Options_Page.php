...
class Options_Page {
	...
	public function enqueue_assets( $post_id ) {
		...
		wp_register_script(
			$this->handle,
			Cgb::plugin_dir_url() . '/js/' . $this->handle . '.min.js',
			array(
				'wp-i18n',
				'lodash',
				'wp-editor',
		...
		wp_set_script_translations( $this->handle, 'cgb', Cgb::plugin_dir_path() . 'languages' );
		wp_enqueue_script( $this->handle );

		wp_register_style(
			$this->handle,
			Cgb::plugin_dir_url() . '/css/' . $this->handle . '.min.css'
		);
