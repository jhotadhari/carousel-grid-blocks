...
Description: Two Gutenbergs image blocks that interact with each other and share the same state.
Version: 0.0.1
Author: jhotadhari
...
class Cgb_Carousel_Grid_Blocks {
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('cgb_on_activate_before_flush');
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('cgb_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		$this->add_roles_and_capabilities();
		do_action('cgb_on_deactivate_before_flush');
		flush_rewrite_rules();
		do_action('cgb_plugin_deactivated');
	}

	public static function on_uninstall() {
		do_action('cgb_plugin_uninstalled');
	}
	...
	protected function init_options() {
		update_option( 'cgb_version', self::VERSION );
		add_option( 'cgb_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'cgb_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-cgb-update.php' );
			// new Cgb_Update();
			// class Cgb_Update()  is missing ??? !!!
		}
	}
...
function cgb_carousel_grid_blocks_init() {
	return Cgb_Carousel_Grid_Blocks::get_instance();
}
cgb_carousel_grid_blocks_init();
