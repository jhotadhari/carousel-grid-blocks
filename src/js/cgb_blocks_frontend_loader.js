...
document.addEventListener('DOMContentLoaded', () => {
	...
	const blockWrapperNodes = document.getElementsByClassName( 'cgb-block-wrapper' );
	if ( blockWrapperNodes.length === 0 ) return;

	let roots = {};

	const getRoot = ( node ) => {

		// console.log( 'debug node', node ); // debug
//
		if ( ! roots.hasOwnProperty( node ) ) {
			const root = ReactDOM.createRoot( node );
			roots = {...roots, [node]: root };
		}
		return roots[node]
	}

	// display placeholder element
	[...blockWrapperNodes].map( ( blockWrapper, idx ) => {
		[... blockWrapper.getElementsByTagName( 'div' )].map( ( block, idx_ ) => {
			const root = getRoot( block );
			root.render(
				<PlaceholderSpinner
					key={ '' + idx + idx_ }
				/>
	...
	loadJS( [cgbBlocks.pluginDirUrl + '/js/cgb_blocks_frontend.min.js'] ).then( () => {
		...
		[...blockWrapperNodes].map( ( blockWrapper ) => {
			...
			[... blockWrapper.getElementsByClassName( 'cgb-grid' )].map( ( grid, idx ) => {
				const root = getRoot( grid );
				root.render(
					<div
						key={ idx }
					>
			...
			[...blockWrapper.getElementsByClassName( 'cgb-carousel' )].map( ( carousel, idx ) => {
				const root = getRoot( carousel );
				root.render(
					<div
						key={ idx }
					>
